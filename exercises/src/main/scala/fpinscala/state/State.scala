...
object RNG {
  ...
  def nonNegativeInt(rng: RNG): (Int, RNG) = {
    val (i, r) = rng.nextInt
    (if (i < 0) -i + Int.MaxValue + Int.MinValue else i, r)
  }
...
object State {
  ...
  def modify[S](f: S => S): State[S, Unit] = for {
    s <- get // Gets the current tests and assigns it to `s`.
    _ <- set(f(s)) // Sets the new tests to `f` applied to `s`.
  } yield ()
